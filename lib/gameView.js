(function () {
  ...
  AsteroidView.prototype.start = function () {
    ...
    if (this.game.mode === 0) {
      this.bgImg.src = 'pics/outer-space.jpg';
    } else if (this.game.mode === 1) {
      this.bgImg.src = 'pics/island.jpg';
    }
    ...
    that.modeLoopID = window.setInterval(function () {
      if (that.game.score > 1000 && that.game.mode !== 1) {
        ...
        that.game.mode = 1;
        $("main").addClass("mode2");
        that.bgImg.src = 'pics/island.jpg';
    ...
    this.intID = window.setInterval(function () {
      ...
      if (that.game.mode === 1) {
        if (that.currGameAudio.currentTime > 9.4) {
          var size = 120 - Math.floor((Math.floor(that.currGameAudio.currentTime * 1000 + 250) % 479)/10);
      ...
      if (that.currGameAudio.currentTime > 38 && !that.hugeAsteroidMade) {
        that.game.addHugeAsteroid();
        that.hugeAsteroidMade = true;
      }
      ...
      that.game.draw(that.ctx, that.bgImg);
